...
可将任意卡片固定为独立的菜单栏项 —— **CPU · GPU · 内存 · 网络 · SSD · 传感器 · 电池** —— 每项都有实时图标与丰富的下拉面板。每一项都可自选**显示形式**（条形 · 历史曲线 · 两行 · 单值 · 图标）以及绘制哪些读数，因此同一指标可以出现两次：CPU 既作条形、又作曲线。全程无需 sudo。
...
> **不支持预发布版 macOS（Beta）。** 本项目只有一台 Mac，且运行正式版，因此无法在此复现或验证
> 开发者 Beta 上的问题。仍然欢迎反馈，而且它们确实有用 —— SiliconScope 读取 Apple 在各版本间
> 会改名的私有 IOReport 接口，早一步发现改名就能在影响所有人之前修复。只是修复会随正式版发布，
> 而不是针对某个 Beta 种子版本。
...
- **[SpectaBooks](https://spectabooks.calidalab.ai/zh-Hans/)** — 为你已有的书而做的阅读器：读取你自己的文件夹、iCloud Drive 或 Google Drive 中的文本、EPUB、PDF 与漫画，阅读位置在 iPhone、iPad 与 Mac 之间同步，朗读与翻译都在设备端完成。
