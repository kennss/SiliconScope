...
任意のカードを独立したメニューバー項目としてピン留めできます — **CPU · GPU · メモリ · ネットワーク · SSD · センサー · バッテリー** — それぞれにライブグリフと充実したドロップダウン。項目ごとに**表示形式**（バー · ヒストリーグラフ · 2 行 · 単一値 · アイコン）と描画する値を選べるので、同じ指標を 2 回並べることもできます: CPU をバーで、さらにグラフで。すべて sudo 不要。
...
**ANE + メモリ帯域**が加わります — 指標名が自分の線の色で塗られているため凡例は不要です。
タイル最下段は**ランタイムが実測した生成速度**と、それがいつの値かです。Ubuntu マシンは
**262 tok/s**、2 台の Mac は **28 tok/s**。現在を表さなくなると値は淡くなります。
This Mac は常に最初のタイルです。*
...
**Intel Mac では**、エージェントは CPU とメモリを報告します——そのマシンが実際に持っているものです。
チップレベルの指標が無いのはハードウェアが無いからです：Neural Engine も Media Engine も無く、
ユニファイドメモリ帯域幅やドメイン別電力も、Apple Silicon だけが公開するインターフェースから来ます。
アプリ本体は引き続き Apple Silicon 専用です。

**Windows マシンでは**、エージェントは CPU とメモリを報告し、NVIDIA カードは Linux と同じ `nvidia-smi`
経路で報告します——使用率、VRAM、温度、電力、プロセスごとの VRAM。Windows には load average が無いため、
そのフィールドは数値を捏造せず空のままです。ドライブ容量はまだ収集していません。このマシンは Linux の
GPU ボックスと同じようにフリートに加わります。ワンライナーのインストーラはまだありません:
`GOOS=windows go build ./agent` でエージェントをビルドし、タスク スケジューラで実行してください。
...
> **リリース前の macOS（ベータ）はサポート対象外です。** このプロジェクトが持つ Mac は 1 台だけで
> 正式版を使っているため、開発者ベータで起きたことをここで再現・検証できません。それでも報告は
> 歓迎ですし実際に役立ってきました — SiliconScope は Apple がビルドごとに改名する非公開の
> IOReport インターフェースを読んでおり、改名を早く知れば全員に届く前に直せます。ただし修正は
> ベータシードではなく正式リリースに合わせて出します。
