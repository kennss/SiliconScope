...
어떤 카드든 자신만의 메뉴바 아이템으로 고정하세요 — **CPU · GPU · 메모리 · 네트워크 · SSD · 센서 · 배터리** — 각각 실시간 글리프와 풍부한 드롭다운을 가집니다. 항목마다 **표시 방식**(막대 · 히스토리 그래프 · 두 줄 · 단일 값 · 아이콘)과 어떤 값을 그릴지 직접 고를 수 있어, 같은 지표를 두 번 띄울 수도 있습니다: CPU를 막대로, 그리고 그래프로. 전부 sudo 불필요.
...
타일 맨 아랫줄은 **런타임이 실제로 측정한 생성 속도**와 그게 언제 값인지입니다. Ubuntu 박스는
**262 tok/s**, 두 Mac은 **28 tok/s**. 현재를 설명하지 못하게 되면 값이 흐려집니다.
This Mac은 항상 첫 타일입니다.*
...
**Intel Mac에서는** 에이전트가 CPU와 메모리를 보고합니다 — 그 기계가 실제로 가진 것들입니다.
칩 레벨 지표가 없는 건 하드웨어가 없기 때문입니다: Neural Engine도 Media Engine도 없고,
...
> **정식 출시 전 macOS(베타)는 지원하지 않습니다.** 이 프로젝트가 가진 Mac은 한 대뿐이고 정식
> 버전을 쓰기 때문에, 개발자 베타에서 일어난 일은 여기서 재현하거나 검증할 수 없습니다. 그래도
> 제보는 환영하고 실제로 도움이 됐습니다 — SiliconScope는 Apple이 빌드마다 이름을 바꾸는 비공개
> IOReport 인터페이스를 읽기 때문에, 이름 변경을 일찍 알면 모두에게 퍼지기 전에 고칠 수 있습니다.
> 다만 수정은 베타 시드가 아니라 정식 출시에 맞춰 나갑니다.
...
- **[SpectaBooks](https://spectabooks.calidalab.ai/ko/)** — 이미 가진 책을 위한 리더 하나. 내 폴더·iCloud Drive·Google Drive의 텍스트·EPUB·PDF·만화를 읽고, 읽던 위치는 iPhone·iPad·Mac에서 이어지며, 소리 내어 읽기와 번역은 기기 안에서.
