...
*가장 풍부한 드롭다운 4종. **GPU / Media / Neural** — GPU, GPU 메모리, ANE, Media를 실시간 막대 + 4선 60초 추세로. **센서** — 유닛별 온도, 실제 **E-Core / P-Core / GPU / Memory** 센서(칩 세대별로 큐레이션한 SMC 키, M1–M5, 그 외는 HID 폴백). **배터리** — 건강도·사이클·상태, SoC 전력 분해, 전력을 많이 쓰는 앱. **SS 콕핏** — 머신 전체를 드롭다운 하나에: 워크로드 판정, 모든 엔진, 60초 추세, 상위 프로세스.*
